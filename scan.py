...
def main():
    ...
    try:
        selfhosted_scan()  # fully isolated: own state/output/alert files; never touches the above
    except Exception as e:  # noqa: BLE001
        print(f"[!] self-hosted module error (ignored, main scan unaffected): {e}")
...
# =====================================================================================
#  SELF-HOSTED PROGRAM DISCOVERY  (fully isolated add-on)
#  Finds independent bug bounty programs NOT on a major platform, that publish a concrete
#  reward table + a submission method. Language-independent detection. Its own files only.
# =====================================================================================

SH_STATE = "data/selfhosted_state.json"
SH_OUT = "docs/data/selfhosted.json"
SH_ALERT, SH_TITLE = "selfhosted.md", "selfhosted_title.txt"
SH_DISCLOSE = "https://raw.githubusercontent.com/disclose/diodb/master/program-list.json"
SH_TOPLIST = "https://s3-us-west-1.amazonaws.com/umbrella-static/top-1m.csv.zip"  # public daily top domains
SH_VERIFY_BATCH, SH_CRAWL_BATCH, SH_RECHECK_DAYS = 40, 180, 30
PLATFORM_DOMAINS = ("hackerone.com", "bugcrowd.com", "intigriti.com", "yeswehack.com",
                    "immunefi.com", "hackenproof.com", "openbugbounty.org", "bugbounty.jp",
                    "yogosha.com", "synack.com", "cobalt.io", "zerocopter.com")

# --- language-independent detectors ---
_CUR = r"(?:[$€£₹¥₦₩]|USD|EUR|GBP|INR|JPY|CAD|AUD|CHF|BRL|ZAR|SGD|NGN|PLN|SEK|NOK|DKK|AED|CNY|RUB|MXN|KRW|TRY)"
SH_AMOUNT = re.compile(r"(?:" + _CUR + r"\s?\d{1,3}(?:[ .,]\d{3})*(?:[.,]\d+)?\s?[kK]?)"
                       r"|(?:\d{1,3}(?:[ .,]\d{3})*(?:[.,]\d+)?\s?[kK]?\s?" + _CUR + r")", re.I)
SH_TIERS = ("critical", "high", "medium", "low", "severe", "severity", "moderate",
            "critique", "élevé", "eleve", "moyen", "faible", "grave", "gravité",
            "crítico", "critico", "crítica", "alto", "alta", "medio", "media", "bajo", "baja",
            "kritisch", "hoch", "mittel", "niedrig", "schwere",
            "médio", "baixo", "elevato", "basso", "hoog", "laag",
            "критический", "высокий", "средний", "низкий",
            "严重", "高危", "中危", "低危", "重大", "심각", "높음", "낮음", "重大", "深刻")
SH_TIME = re.compile(r"\b\d{1,3}\s?(?:hours?|hrs?|days?|business\s?days?|weeks?|months?|"
                     r"jours?|ouvr[ée]s?|d[ií]as?|h[aá]biles|werktage|tage|stunden|wochen|"
                     r"settimane|giorni|semanas|dni|godzin|dagen|dager|時間|日|주|일)\b", re.I)
SH_RESP = ("respond", "response", "acknowled", "triage", "reply", "we aim", "within",
           "répond", "réponse", "délai", "respuesta", "responder", "plazo", "antwort",
           "risposta", "resposta", "prazo", "antwoord", "sla", "ответ", "回复", "対応", "응답")
SH_EMAIL = re.compile(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}")
SH_BADMAIL = ("example.", "sentry", "wixpress", "@2x", "noreply", "no-reply", ".png", ".jpg",
              "@your", "ingest.", "@sentry.io", ".gif", ".svg", "u003e", "protonmail.com/@")


def _bad_email(e):
    e = e.lower()
    if any(b in e for b in SH_BADMAIL):
        return True
    local = e.split("@")[0]
    return bool(re.fullmatch(r"[0-9a-f]{24,}", local))  # tracking-hash local part, not a real inbox
SH_SECPREFIX = ("security@", "psirt@", "report@", "vuln", "abuse@", "disclosure@", "bugbounty@", "bounty@")


def _amount_val(tok):
    digits = re.sub(r"[^\d]", "", tok.split(".")[0] if tok.count(".") == 1 and len(tok.split(".")[-1]) <= 2 else tok)
    v = int(digits) if digits else 0
    return v * 1000 if re.search(r"\d\s?[kK]\b", tok) else v


def sh_reward(text):
    """Strictest: named money amounts tied to severity tiers. Returns (ok, samples)."""
    toks = [t.strip() for t in SH_AMOUNT.findall(text)]
    amts = list(dict.fromkeys(t for t in toks if t))
    big = [t for t in amts if _amount_val(t) >= 100]
    low = text.lower()
    has_tier = any(w in low for w in SH_TIERS)
    ok = len(big) >= 2 and (has_tier or len(big) >= 3)
    return ok, big[:6]


def sh_sla(text):
    low = text.lower()
    for m in SH_TIME.finditer(text):
        s = low[max(0, m.start() - 90):m.end() + 90]
        if any(w in s for w in SH_RESP):
            return True, m.group(0).strip()
    return False, ""


def sh_submission(text, page_url, seed_email):
    if seed_email and not _bad_email(seed_email):
        return True, seed_email
    cands = [e for e in SH_EMAIL.findall(text) if not _bad_email(e)]
    pref = [e for e in cands if any(e.lower().startswith(p) or p in e.lower() for p in SH_SECPREFIX)]
    if pref:
        return True, pref[0]
    if cands:
        return True, cands[0]
    if re.search(r'href=["\'][^"\']*(?:report|submit|disclos|vulnerab)[^"\']*["\']', text, re.I) \
       or re.search(r"(?:report|submit)\s+(?:a\s+)?(?:vulnerab|bug|issue|security)", text, re.I):
        return True, "web form: " + page_url
    return False, ""


def sh_on_platform(url, text):
    host = re.sub(r"^https?://", "", url or "").split("/")[0].lower()
    if any(host == p or host.endswith("." + p) for p in PLATFORM_DOMAINS):
        return True
    subs = re.findall(r'href=["\']https?://([^/"\']+)/[^"\']*(?:report|submit|security|disclos)', text or "", re.I)
    return bool(subs) and all(any(h.endswith(p) for p in PLATFORM_DOMAINS) for h in subs)


def sh_verify(domain, cand):
    """Fetch policy page and decide. Returns a verified record or None."""
    url = cand.get("policy") or ("https://" + domain)
    try:
        body, h = get(url, limit=800_000, timeout=12)
    except Exception:  # noqa: BLE001
        return None
    if "text/html" not in h.get("Content-Type", "text/html") and "text/plain" not in h.get("Content-Type", ""):
        return None
    raw = body.decode("utf-8", "ignore")
    text = re.sub(r"<script.*?</script>|<style.*?</style>", " ", raw, flags=re.S | re.I)
    if sh_on_platform(url, raw):
        return None
    ok_r, rewards = sh_reward(text)
    if not ok_r:
        return None
    ok_s, submit = sh_submission(raw, url, cand.get("email"))
    if not ok_s:
        return None
    ok_sla, sla = sh_sla(text)
    return {"name": cand.get("name") or domain, "domain": domain, "policy": url,
            "submit": submit, "rewards": rewards, "sla": sla if ok_sla else "",
            "source": cand.get("source", "?")}


def sh_securitytxt(domain):
    """Discover an independent program via /.well-known/security.txt -> Policy + Contact."""
    for base in ("https://" + domain, "https://www." + domain):
        for path in ("/.well-known/security.txt", "/security.txt"):
            try:
                body, h = get(base + path, limit=60_000, timeout=7)
            except Exception:  # noqa: BLE001
                continue
            txt = body.decode("utf-8", "ignore")
            if "Contact:" not in txt and "contact:" not in txt:
                continue
            pol = re.search(r"(?im)^Policy:\s*(\S+)", txt)
            con = re.search(r"(?im)^Contact:\s*(\S+)", txt)
            email = ""
            if con:
                m = SH_EMAIL.search(con.group(1))
                email = m.group(0) if m else ""
            return {"policy": pol.group(1) if pol else base, "email": email,
                    "name": domain, "source": "security.txt"}
    return None


def sh_discover_disclose(state):
    try:
        body, _ = get(SH_DISCLOSE, timeout=60)
        rows = json.loads(body)
    except Exception as e:  # noqa: BLE001
        print(f"[!] disclose.io fetch: {e}")
        return
    added = 0
    for r in rows:
        if not r.get("offers_bounty"):
            continue
        pol = r.get("policy_url") or ""
        host = re.sub(r"^https?://", "", pol).split("/")[0].lower()
        if not host or any(host.endswith(p) for p in PLATFORM_DOMAINS):
            continue
        if json.dumps(r).lower().count("hackerone.com") or "bugcrowd.com" in json.dumps(r).lower():
            continue
        dom = host[4:] if host.startswith("www.") else host
        if dom in state["known"]:
            continue
        state["queue"].append(dom)
        state["cand"][dom] = {"policy": pol, "email": r.get("contact_email") or "",
                              "name": r.get("program_name") or dom, "source": "disclose.io"}
        state["known"][dom] = TODAY
        added += 1
    print(f"[sh] disclose.io: {added} new candidates")


def sh_discover_toplist(state):
    """Crawl a rolling window of top domains for security.txt to find NEW independent programs."""
    import io
    import zipfile
    try:
        body, _ = get(SH_TOPLIST, timeout=90)
        zf = zipfile.ZipFile(io.BytesIO(body))
        lines = zf.read(zf.namelist()[0]).decode("utf-8", "ignore").splitlines()
    except Exception as e:  # noqa: BLE001
        print(f"[!] toplist fetch: {e}")
        return
    off = state.get("crawl_off", 0)
    window = lines[off:off + SH_CRAWL_BATCH]
    if not window:
        off = 0
        window = lines[:SH_CRAWL_BATCH]
    state["crawl_off"] = off + SH_CRAWL_BATCH
    doms = [ln.split(",")[-1].strip().lower() for ln in window if "," in ln]
    doms = [d for d in doms if d and d not in state["known"] and not any(d.endswith(p) for p in PLATFORM_DOMAINS)]
    found = 0
    with ThreadPoolExecutor(16) as ex:
        for dom, res in zip(doms, ex.map(sh_securitytxt, doms)):
            if res and dom not in state["known"]:
                state["known"][dom] = TODAY
                state["cand"][dom] = res
                state["queue"].append(dom)
                found += 1
    print(f"[sh] security.txt crawl [{off}:{off + SH_CRAWL_BATCH}]: {found} candidates")


def sh_scan_impl(state):
    state.setdefault("known", {}); state.setdefault("cand", {})
    state.setdefault("queue", []); state.setdefault("verified", {}); state.setdefault("crawl_off", 0)
    if not state.get("seeded"):
        sh_discover_disclose(state)
        state["seeded"] = TODAY
    elif NOW.hour < 3:  # refresh disclose.io roughly once a day
        sh_discover_disclose(state)
    sh_discover_toplist(state)

    # re-queue verified programs due for a re-check
    for d, rec in list(state["verified"].items()):
        if rec.get("last", "") < _days_ago(SH_RECHECK_DAYS) and d not in state["queue"]:
            state["queue"].append(d)

    batch, state["queue"] = state["queue"][:SH_VERIFY_BATCH], state["queue"][SH_VERIFY_BATCH:]
    new_hits = []
    with ThreadPoolExecutor(8) as ex:
        results = list(ex.map(lambda d: (d, sh_verify(d, state["cand"].get(d, {}))), batch))
    for d, rec in results:
        if rec:
            first = state["verified"].get(d, {}).get("first", TODAY)
            was = d in state["verified"]
            rec.update(first=first, last=TODAY)
            state["verified"][d] = rec
            if not was:
                new_hits.append(rec)
        elif d in state["verified"]:
            del state["verified"][d]  # no longer qualifies (table/table submission removed)

    ver = sorted(state["verified"].values(), key=lambda r: r["first"], reverse=True)
    save(SH_OUT, {"updated": NOW.isoformat(timespec="minutes"), "count": len(ver),
                  "queued": len(state["queue"]), "programs": ver})
    save(SH_STATE, state)
    print(f"[sh] verified total {len(ver)} | {len(new_hits)} new | queue {len(state['queue'])}")
    sh_write_alert(new_hits)


def sh_write_alert(hits):
    for f in (SH_ALERT, SH_TITLE):
        if os.path.exists(f):
            os.remove(f)
    if not hits:
        return
    md = [f"# New self-hosted bounty programs {TODAY}\n",
          f"{len(hits)} independent program(s) found with a concrete reward table, off the major platforms.\n",
          "Dashboard: https://abdulsalam-create.github.io/bounty-watch/#selfhosted\n"]
    for r in hits:
        md.append(f"### {r['name']}\n{r['policy']}")
        md.append(f"- rewards: {', '.join('`' + x + '`' for x in r['rewards'])}")
        md.append(f"- submit: {r['submit']}" + (f"\n- SLA: {r['sla']}" if r["sla"] else ""))
        md.append("")
    with open(SH_ALERT, "w", encoding="utf-8") as f:
        f.write("\n".join(md) + "\ncc @abdulsalam-create")
    with open(SH_TITLE, "w", encoding="utf-8") as f:
        f.write(f"Self-hosted programs {TODAY}: {len(hits)} new independent target(s)")
